      ...
      <div class="changeButton">
            ...
            <template v-slot:headButton>
              <el-badge :value="imageList.length" class="item">
                <el-button
                  ...
                  v-tip.sure.right="
                    'ctrl+F唤起/隐藏列表 列表出现之后，点击蒙层可以隐藏列表'
                  "
                >
                  {{ $t('general.selected') }}
                </el-button>
        ...
        >
          <el-radio-button label="true">
            <svg-icon icon-class="jiantou" style="font-size: 12px;"></svg-icon>
          </el-radio-button>
          <el-radio-button label="false">
            <svg-icon
              icon-class="jiantou"
              style="transform:rotate(90deg); font-size: 12px ;display:block"
            ></svg-icon>
          </el-radio-button>
        </el-radio-group>
        <el-button
          ...
          :title="
            $t(
              status
                ? 'imageLineCompare.hideLine'
                : 'imageLineCompare.displayLine'
            )
          "
        >
          <svg-icon
...
export default {
  ...
  methods: {
    ...
    // 双击某还未选择的图片后，将其替换掉已选图片中较早选择的图片，即始终保持已选择图片为最后选择的两张
    handleDbclick(path) {
      if (this.selectedList.includes(path)) {
        if (this.selectedList[0] === path) {
          this.selectedList = [this.selectedList[1], path];
        }
      } else if (this.imageList.includes(path)) {
